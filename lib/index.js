...
function RpcClient(opts) {
    ...
    this.port = opts.port || 9953;
    this.user = opts.user || 'koenpeters';
    this.pass = opts.pass || 'koenislekker';
...
function innerRpc(request, callback) {
    ...
    var errorMessage = 'PACGlobal JSON-RPC: ';
    ...
    var req = this.protocol.request(options, function (res) {
        ...
        res.on('end', function () {
            ...
            if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
                var exceededError = new Error('PACGlobal JSON-RPC: ' + buf.toString('utf8'));
